...
$laws_type = get_field('laws_type', $post->ID );
...
if ( !empty ( $huuliin_surwalj ) ) {
    $huuliin_surwalj_output .= '<a href="' . esc_url($huuliin_surwalj_url) . '" class="uk-button with-icon laws-btn" target="_blank">';
            ...
            <div class="content-area <?php echo $sidebar === 'enable' ? 'uk-width-expand' : 'uk-width-1-1'?>">
                <article <?php post_class('single'); ?>>
                    <div class="entry-post nt-page-title"><?php 
                        ...
                        echo '<div class="law-page-header uk-text-center">';
                            echo '<h4 class="law-page-title">';
                                echo get_the_title();
                            echo '</h4>';
                        ...
                        if  ( $laws_type == 'default' ) {
                            echo '<div class="laws-container uk-text-center">';
                            if ( !empty ( $huuliin_image ) ) {
                                echo '<img class="laws-image" src="' . $huuliin_image . '" alt="' . get_the_title() . '" />';
                            }
                            if ( !empty ( $huuliin_togtool ) ) {
                                echo '<h3 class="laws-title">';
                                    echo $huuliin_togtool;
                                echo '</h3>';
                            }

                            if ( !empty ( $huuliin_date ) ) {
                                echo '<div class="laws-meta uk-flex uk-flex-between">';
                                $dates = explode( ',', $huuliin_date );
                                    echo '<span class="laws-date">';
                                        echo $dates[0] . ' оны ' . $dates[1] . ' дугаар сарын ' . $dates[2] . '-ны өдөр';
                                    echo '</span>';
                                    echo '<span class="laws-location">' . $huuliin_location . '</span>';
                                echo '</div>';
                            }
                            if ( !empty ( $huuliin_dugaar ) ) {
                                echo '<h6 class="laws-number">';
                                    echo $huuliin_dugaar;
                                echo '</h6>';
                            }
                            echo '</div>';
                        ...
                        if ( $laws_type == 'default' ) {
                            the_content();
                            echo '<div class="laws-container-bottom uk-text-center">';
                                if ( !empty ( $huuliin_darga ) ) {
                                    echo '<h3 class="laws-darga">';
                                        echo $huuliin_darga;
                                    echo '</h3>';
                                }
                                if ( !empty ( $huuliin_darga_name ) ) {
                                    echo '<h3 class="laws-darga-name">';
                        ...
                        echo '<div class="law-page-footer uk-flex uk-flex-column uk-flex-middle '. ($laws_type == 'default' ? 'margin-top-small' : '') .'">';
                            
                        
                        // if ( $huuliin_surwalj_output ) {
                        //         echo $huuliin_surwalj_output;
                        //     }
                            if ( ! empty ( $huuli_pdf ) ) {
                                echo '<a href="' . esc_url( $huuli_pdf['url'] ) . '" class="uk-button with-icon laws-download-btn" download="' . esc_url( $huuli_pdf['url'] ) . '" target="_blank">';
