...
$atts = array_merge(array(
    ...
    'element_atts' => array(
        'class' => array('tw-element tw-news-filter tw-infinite-container'),
        'data-item-selector'=>array('.news-item'),
    ),
    'animation_target' => 'article.post',
), vc_map_get_attributes($this->getShortcode(), $atts));

$filter_text     = empty( $_REQUEST['filter_text'] )     ? '' : trim( mb_convert_case( $_REQUEST['filter_text'], MB_CASE_LOWER, "UTF-8" ) );
$filter_cat      = empty( $_REQUEST['filter_cat'] )      ? '' : trim( $_REQUEST['filter_cat'] );
$filter_tag      = empty( $_REQUEST['filter_tag'] )      ? '' : trim( $_REQUEST['filter_tag'] );
...
list( $output ) = lvly_item( $atts );
    if ( defined( 'ICT_PORTAL_MAIN' ) ) {
         ...
         */
        $tw_cache_created = 0;
        $tw_all_sites_latest_posts_default = array();

        // Current site Posts
        $tw_cache_expire_time = 60 * 30;
        $tw_cache_name        = 'tw_all_sites_latest_posts_default_' . 'portal';
        ...
        // Merge Datas
        $tw_all_sites_latest_posts_default = array_merge( $tw_all_sites_latest_posts_default, $crr_new_posts );
        ...
        foreach( $tw_get_subsite_urls as $tw_get_subsite_url ) {
            $tw_cache_expire_time = 60 * 30;
            $tw_cache_name        = 'tw_all_sites_latest_posts_default_' . esc_sql( esc_url( $tw_get_subsite_url ) );
            $tw_cache_data        = get_transient( $tw_cache_name );
            $tw_cache_data        = $tw_cache_data && is_array( $tw_cache_data ) ? $tw_cache_data : array();
            $tw_cache_not_expired = get_transient( $tw_cache_name . '_not_expired' );
                        ...
                        if ( ! empty( $response['body'] ) ) {
                            ...
                            if ( is_array( $response_body ) ) {
                                foreach( $response_body as $sub_site_post ) {
                                    ...
                                    } elseif( ! empty( $sub_site_post['images'] ) && ! empty( $sub_site_post['images']['lvly_news_carousel'] ) ) {
                                        $new_post['images']['lvly_carousel_3'] = $sub_site_post['images']['lvly_news_carousel'];
                                    }
                                    ...
                                    // Tags
                                    $new_post['tags']      = !empty( $sub_site_post['tags'] ) ? $sub_site_post['tags'] : array();
        ...
        array_multisort(array_map(function($element) {
            return $element['date'];
        }, $tw_all_sites_latest_posts_default), SORT_DESC, $tw_all_sites_latest_posts_default);



        // Filter Options
        $filter_cat_options = $filter_tag_options = array();

        foreach ( $tw_all_sites_latest_posts_default as $crr_post ) {
            if ( ! empty( $crr_post['cats'] ) ) {
                foreach ( $crr_post['cats'] as $crr_post_cat ) {
                    if ( ! empty( $crr_post_cat['name'] ) ) {
                        $filter_cat_options[ esc_attr( $crr_post_cat['name'] ) ] = esc_attr( $crr_post_cat['name'] );
                    }
                }
            }
            if ( ! empty( $crr_post['tags'] ) ) {
                foreach ( $crr_post['tags'] as $crr_post_tag ) {
                    if ( ! empty( $crr_post_tag['name'] ) ) {
                        $filter_tag_options[ esc_attr( $crr_post_tag['name'] ) ] = esc_attr( $crr_post_tag['name'] );
                    }
                }
            }
        }

        ksort( $filter_cat_options );
        ksort( $filter_tag_options );
                ...
                $output .= '<div class="uk-width-3-4 uk-flex uk-flex-right">';
                    ...
                    $output .= '<div class="tw-field-outer">';
                            ...
                            $output .= '<select class="tw-field" name="filter_tag">';
                                ...
                                foreach ( $filter_tag_options as $key => $val ) {
                                    $output .= '<option value="' . esc_attr( $key ) . '"' . ( $key === $filter_tag ? ' selected="selected"' : '' ) . '>' . esc_html__( $val ) . '</option>';
                                }
                            $output .= '</select>';
                    $output .= '</div>';
                    ...
                    $output .= '<div class="tw-field-outer">';
                        ...
                        $output .= '<select class="tw-field" name="filter_cat">';
                            $output .= '<option value="">' . esc_html__( 'Сонгох' ,'lvly') . '</option>';
                            foreach ( $filter_cat_options as $key => $val ) {
                                $output .= '<option value="' . esc_attr( $key ) . '"' . ( $key === $filter_cat ? ' selected="selected"' : '' ) . '>' . esc_html__( $val ) . '</option>';
                            }
                        $output .= '</select>';
                    $output .= '</div>';
        ...
        // Filter Category
        if ( ! empty( $filter_cat ) ) {
            $tw_all_sites_latest_posts_default = array_filter( $tw_all_sites_latest_posts_default, function( $item ) use ( $filter_cat ) {
                $result = false;
                if ( ! empty( $item['cats'] ) && is_array( $item['cats'] ) ) {
                    foreach ( $item['cats'] as $cat_item ) {
                        if ( ! empty( $cat_item['name'] ) && $cat_item['name'] === $filter_cat ) {
                            $result = true;
                            break;
                        }
                    }
        ...
        // Filter Tag
        if ( ! empty( $filter_tag ) ) {
            $tw_all_sites_latest_posts_default = array_filter( $tw_all_sites_latest_posts_default, function( $item ) use ( $filter_tag ) {
                $result = false;
                if ( ! empty( $item['tags'] ) && is_array( $item['tags'] ) ) {
                    foreach ( $item['tags'] as $tag_item ) {
                        if ( ! empty( $tag_item['name'] ) && $tag_item['name'] === $filter_tag ) {
                            $result = true;
                            break;
                        }
                    }
        ...
        if ( ! empty( $filter_text ) ) {
            $tw_all_sites_latest_posts_default = array_filter( $tw_all_sites_latest_posts_default, function( $item ) use ( $filter_text ) {
                $result = false;
                if ( ! empty( $item['title'] ) && strpos( mb_convert_case( $item['title'], MB_CASE_LOWER, "UTF-8" ), $filter_text ) !== false ) {
                    // $filter_text
                    $result = true;
                }
                return $result;
        ...
        $max_num_pages = ceil( count( $tw_all_sites_latest_posts_default ) / $posts_per_page );
        $tw_all_sites_latest_posts_default = array_slice( $tw_all_sites_latest_posts_default, $offset, $posts_per_page );
                        ...
                        $output .= ' <div class="entry-post uk-height-1-1">';
                            $output .= '<div class="blog-post-container">';
                                $output .= '<div class="news-filterthubnail">';
                                    $image =  ! empty( $crr_post['images'] ) && ! empty( $crr_post['images']['lvly_carousel_3'] ) ? $crr_post['images']['lvly_carousel_3'] : array() ;
                                    if ( ! empty( $image['url'] ) ) {
                                        $output .= '<a href="' . esc_url( empty( $crr_post['permalink'] ) ? '#' : $crr_post['permalink'] ) . '" title="' . esc_attr( empty( $crr_post['title'] ) ? '' : $crr_post['title'] ) . '" style="background-image:url('. esc_url( $image['url'] ) .')">';
                                            $output .= '<img src="' . esc_url( $image['url'] ) . '" alt="' . esc_attr( empty( $image['alt'] ) ? '' : $image['alt'] ) . '" />';
                                        $output .= '</a>';
                                    }
                                $output .= '</div>';
                                $output .= '<div class="news-filtercontent">';
                                    $output .= '<div class="tw-news-filter uk-flex">';
                                        ...
                                        if ( ! empty( $crr_post['cats'] ) && is_array( $crr_post['cats'] ) ) {
                                            $output .= '<div class="tw-news-filter-cats uk-flex">';
                                                foreach ( $crr_post['cats'] as $cr_cat ) {
                                                    if ( ! empty( $cr_cat['name'] ) ) {
                                                        $output .= '<div class="cat-item"><span># ' . esc_html( $cr_cat['name'] ) . '</span></div>';
                                                    }
                                                }
                                            $output .= '</div>';
                                        }
                                    ...
                                    $output .= '<h2 class="news-filtertitle">';
                                        $output .= '<a href="' . esc_url( empty( $crr_post['permalink'] ) ? '#' : $crr_post['permalink'] ) . '">' . ( empty( $crr_post['title'] ) ? '' : $crr_post['title'] ) . '</a>';
                                    $output .= '</h2>';
                                $output .= '</div>';
                            $output .= '</div>';
                        $output .= '</div>';
            ...
            $output .= '<div class="tw-filter-container" data-uk-grid>';
                ...
                $output .= '<div class="uk-width-3-4 uk-flex uk-flex-right">';
                    ...
                    $output .= '<div class="tw-field-outer">';
                            ...
                            $output .= '<select class="tw-field" name="filter_tag">';
                                $output .= '<option value="">' . esc_html__( 'Сонгох' ,'lvly') . '</option>';
                                $term_post_tags = get_terms('post_tag');
                                foreach ( $term_post_tags as $term_post_tag ) {
                                    $output .= '<option value="' . esc_attr( $term_post_tag -> term_id ) . '"' . ( $term_post_tag -> term_id === $filter_tag ? ' selected="selected"' : '' ) . '>' . esc_html__( $term_post_tag -> name ) . '</option>';
                                }
                            $output .= '</select>';
                    $output .= '</div>';
                    ...
                    $output .= '<div class="tw-field-outer">';
                        ...
                        $output .= '<select class="tw-field" name="filter_cat">';
                            $output .= '<option value="">' . esc_html__( 'Сонгох' ,'lvly') . '</option>';
                            $term_post_cats = get_terms('category');
